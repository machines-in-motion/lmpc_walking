...
def compute_recursive_dynamics(P_ps, P_vs, P_zs, P_pu, P_vu, P_zu, N, x_hat_k, y_hat_k, U):
    X         = np.zeros((N,3))
    Y         = np.zeros((N,3))
    ...
    X[0:N,2]  = np.dot(P_zs, x_hat_k) + np.dot(P_zu, U[0:N])   #x_ddot

    Y[0:N,0]  = np.dot(P_ps, y_hat_k) + np.dot(P_pu, U[N:2*N]) #y
    Y[0:N,1]  = np.dot(P_vs, y_hat_k) + np.dot(P_vu, U[N:2*N]) #y_Dot
    Y[0:N,2]  = np.dot(P_zs, y_hat_k) + np.dot(P_zu, U[N:2*N]) #y_ddot
